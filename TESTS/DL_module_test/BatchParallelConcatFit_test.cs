...
internal static class BatchParallelConcatFit_test
{
    public static bool Run()
    {
        ...
        var model = new BatchParallelConcatModel<float>(
            new BatchSequential<float>(
                new BatchConvolution2DLayer<float>(1, 2, kernelSize: 3),
                new BatchReLUActivation<float>(),
                new BatchGlobalAveragePool2DLayer<float>(),
                new BatchFullyConnectedLayer<float>(2, 1)),
            new BatchSequential<float>(
                new BatchDepthwiseConvolution2DLayer<float>(1, kernelSize: 3),
                new BatchReLUActivation<float>(),
                new BatchGlobalAveragePool2DLayer<float>(),
                new BatchFullyConnectedLayer<float>(1, 1)));
        ...
        trainer.FitBatchParallelConcat(
            model,
