...
public static partial class Dft3D
{
    ...
    /// </summary>
    /// <param name="input">输入数据（线性展平的 3D <see cref="ComplexFp32"/>），长度至少为 nx*ny*nz。</param>
    /// <param name="output">输出数据（线性展平的 3D <see cref="ComplexFp32"/>），长度至少为 nx*ny*nz。</param>
    ...
    public static void Forward(ReadOnlySpan<ComplexFp32> input, Span<ComplexFp32> output, int nx, int ny, int nz, fftw_flags flags = fftw_flags.Estimate)
        => Dft3DComplex(input, output, nx, ny, nz, fftw_direction.Forward, flags);

    /// <summary>
    /// 执行三维复数到复数的正向傅里叶变换（C2C，PinnableArray 重载）。
    /// </summary>
    /// <param name="input">已固定的输入缓冲区（线性展平的 3D <see cref="ComplexFp32"/>），长度至少为 nx*ny*nz。</param>
    /// <param name="output">已固定的输出缓冲区（线性展平的 3D <see cref="ComplexFp32"/>），长度至少为 nx*ny*nz。</param>
    /// <param name="nx">X 维度长度。</param>
    /// <param name="ny">Y 维度长度。</param>
    /// <param name="nz">Z 维度长度。</param>
    /// <param name="flags">规划策略，默认 <see cref="fftw_flags.Estimate"/>。</param>
    ...
    /// </summary>
    /// <param name="input">输入数据，长度至少为 nx*ny*nz。</param>
    /// <param name="output">输出数据，长度至少为 nx*ny*nz。</param>
    /// <param name="nx">X 维度长度。</param>
    /// <param name="ny">Y 维度长度。</param>
    /// <param name="nz">Z 维度长度。</param>
    /// <param name="flags">规划策略，默认 <see cref="fftw_flags.Estimate"/>。</param>
    public static void Inverse(ReadOnlySpan<ComplexFp32> input, Span<ComplexFp32> output, int nx, int ny, int nz, fftw_flags flags = fftw_flags.Estimate)
    ...
    /// 注意：FFTW 的逆变换不执行归一化；如需 1/(nx*ny*nz) 缩放请在外部处理。
    /// </summary>
    /// <param name="input">已固定的输入缓冲区，长度至少为 nx*ny*nz。</param>
    /// <param name="output">已固定的输出缓冲区，长度至少为 nx*ny*nz。</param>
    /// <param name="nx">X 维度长度。</param>
    /// <param name="ny">Y 维度长度。</param>
    /// <param name="nz">Z 维度长度。</param>
    /// <param name="flags">规划策略，默认 <see cref="fftw_flags.Estimate"/>。</param>
    ...
    /// </summary>
    /// <param name="input">输入数据缓冲区（线性展平的 3D <see cref="ComplexFp32"/> 数据），长度至少为 nx*ny*nz。</param>
    /// <param name="output">输出数据缓冲区（线性展平的 3D <see cref="ComplexFp32"/> 数据），长度至少为 nx*ny*nz。</param>
    /// <param name="nx">X 维度大小。</param>
    /// <param name="ny">Y 维度大小。</param>
    /// <param name="nz">Z 维度大小。</param>
    ...
    internal static void Dft3DComplex(ReadOnlySpan<ComplexFp32> input, Span<ComplexFp32> output, int nx, int ny, int nz, fftw_direction direction, fftw_flags flags = fftw_flags.Estimate)
    {
        ...
        if (input.Length < total)
            throw new ArgumentException("Input span length must be at least nx * ny * nz.", nameof(input));
        if (output.Length < total)
            throw new ArgumentException("Output span length must be at least nx * ny * nz.", nameof(output));
    ...
    /// </summary>
    /// <param name="input">已固定的输入缓冲区（线性展平的 3D <see cref="ComplexFp32"/> 数据），长度至少为 nx*ny*nz。</param>
    /// <param name="output">已固定的输出缓冲区（线性展平的 3D <see cref="ComplexFp32"/> 数据），长度至少为 nx*ny*nz。</param>
    /// <param name="nx">X 维度大小。</param>
    /// <param name="ny">Y 维度大小。</param>
    /// <param name="nz">Z 维度大小。</param>
    /// <param name="direction">变换方向，见 <see cref="fftw_direction"/>。</param>
    ...
    internal static void Dft3DComplex(PinnableArray<ComplexFp32> input, PinnableArray<ComplexFp32> output, int nx, int ny, int nz, fftw_direction direction, fftw_flags flags = fftw_flags.Estimate)
    {
        ...
        var total = nx * ny * nz;
        if (input.Length < total)
            throw new ArgumentException("Input buffer length must be at least nx * ny * nz.", nameof(input));
        if (output.Length < total)
            throw new ArgumentException("Output buffer length must be at least nx * ny * nz.", nameof(output));
