        ...
        where T : IBinaryFloatingPointIeee754<T>
    {
        ...
        Parallel.For(0, input.Depth, (int d) =>
        {
            for (int y = 0; y < input.Height; y++)
            {
                ...
                {
                    normalized[x, y, d] = (input[x, y, d] - mean.W[d]) /   T.Sqrt(v + variance.W[d]) * scale.W[d] + shift.W[d];
                }
            }
        });
        return normalized;
    ...
    public static Tensor BatchNorm(Tensor input, Tensor mean, Tensor variance, Tensor shift, Tensor scale)
    {
        ...
        Parallel.For(0, input.Depth, (int d) =>
        {
            for (int y = 0; y < input.Height; y++)
            {
                for (int x = 0; x < input.Width; x++)
                {
                    normalized[x, y, d] = (input[x, y, d] - mean.W[d]) /   MathF.Sqrt(v + variance.W[d]) * scale.W[d] + shift.W[d];
                }
            }
        });
        return normalized;
