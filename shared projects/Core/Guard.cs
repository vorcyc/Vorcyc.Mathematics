...
internal static class Guard
{
    ...
    /// <param name="arg">Argument (number)</param>
    /// <param name="argName">Argument name</param>
    public static void AgainstNonPositive<T>(T arg, string argName = "argument")
        where T : unmanaged, IFloatingPointIeee754<T>, IMinMaxValue<T>
    {
        if (arg < T.CreateChecked(1e-30))
        {
            throw new ArgumentException($"{argName} must be positive!");
        }
    }
    ...
    /// <param name="high">Upper boundary of the range</param>
    /// <param name="valueName">Argument name</param>
    public static void AgainstInvalidRange(float value, float low, float high, string valueName = "value")
    {
        if (value < low || value > high)
        {
            throw new ArgumentException($"{valueName} must be in range [{low}, {high}]!");
    ...
    /// <param name="lowName">Name of the first argument</param>
    /// <param name="highName">Name of the second argument</param>
    public static void AgainstInvalidRange(float low, float high, string lowName = "low", string highName = "high")
    {
        if (high - low < 1e-30f)
        {
            throw new ArgumentException($"{highName} must be greater than {lowName}!");
    ...
    /// <param name="argName">Argument name</param>
    public static void AgainstNotOrdered(float[] values, string argName = "Values")
    {
    ...
    /// <param name="desired">Desired magnitude response (gains)</param>
    /// <param name="weights">Weights</param>
    public static void AgainstIncorrectFilterParams(float[] freqs, float[] desired, float[] weights)
    {
        var n = freqs.Length;
