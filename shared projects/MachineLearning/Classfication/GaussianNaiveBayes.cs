...
/// <summary>
/// Gaussian naive Bayes classifier, suitable for continuous numeric features.
/// </summary>
    ...
    where T : struct, IFloatingPointIeee754<T>
{
    ...
    /// </summary>
    /// <param name="varianceFloor">Variance floor used to avoid division by zero.</param>
    /// <param name="context">Execution policy context; when null the ambient scope or default context is used.</param>
    public GaussianNaiveBayes(T varianceFloor = default, ComputingContext? context = null)
    {
    ...
    /// <summary>
    /// Fits the model. Labels must be non-negative integers.
    /// </summary>
        ...
        if (ComputingContextExecution.UseParallelIndexed(Context, rows, (long)cols * numClasses))
        {
            ...
            ComputingContextExecution.ForEach(
                ...
                i =>
                {
                    ...
                    for (int j = 0; j < cols; j++)
                        localSample[j] = x[i, j];
                    var localScores = new T[numClasses];
                    ComputeLogPosteriors(localSample, localScores);
                    buffer[i] = ClassificationMath.ArgMaxClassScores(_classLabels, localScores);
                },
    ...
    /// <summary>
    /// Returns the posterior probability of each class (normalized).
    /// </summary>
    ...
    private void ComputeLogPosteriors(T[] sample, Span<T> destination)
    {
        ...
        if (destination.Length < _classLabels.Length)
            throw new ArgumentException("The destination span is too short.", nameof(destination));
