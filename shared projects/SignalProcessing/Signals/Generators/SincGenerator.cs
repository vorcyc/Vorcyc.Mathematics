using Vorcyc.Mathematics;

namespace Vorcyc.Mathematics.SignalProcessing.Signals.Generators;

/// <summary>
/// Scaled sinc signal generator.
/// </summary>
public sealed class SincGenerator : IAmplitudeOscillator
{
    private int _n;

    /// <inheritdoc />
    public float SamplingRate { get; set; } = 44100f;

    /// <inheritdoc />
    public float Frequency { get; set; } = 100f;

    /// <inheritdoc />
    public float Min { get; set; } = -1f;

    /// <inheritdoc />
    public float Max { get; set; } = 1f;

    /// <inheritdoc />
    public float Phase { get; set; }

    /// <inheritdoc />
    public float NextSample()
    {
        var sample = Min + (Max - Min) * TrigonometryHelper.Sinc(_n * Frequency / SamplingRate);
        _n++;
        return sample;
    }

    /// <inheritdoc />
    public void Reset() => _n = 0;
}
