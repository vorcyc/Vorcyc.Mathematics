...
public class BinauralPanEffect : StereoEffect
{
                             ...
                             float[][][] rightHrirs)
    {
        ...
        _leftEarConvolver = new OlsBlockConvolver(_leftEarHrir, (4 * hrirLength).NextPowerOf2());
        _rightEarConvolver = new OlsBlockConvolver(_rightEarHrir, (4 * hrirLength).NextPowerOf2());

        // crossover freq = 0.01 by default: e.g. 160 Hz with 16kHz sampling rate
        // (by default, crossover filtering is turned off)

        _crossoverLpFilterLeft = new Filters.BiQuad.LowPassFilter(0.01f);
        _crossoverHpFilterLeft = new Filters.BiQuad.HighPassFilter(0.01f);
        _crossoverLpFilterRight = new Filters.BiQuad.LowPassFilter(0.01f);
        _crossoverHpFilterRight = new Filters.BiQuad.HighPassFilter(0.01f);
