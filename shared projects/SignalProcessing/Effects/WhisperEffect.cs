...
public class WhisperEffect : OverlapAddFilter
{
                                            ...
                                            float[] filteredIm)
    {
        ...
        {
            var mag = MathF.Sqrt(re[j] * re[j] + im[j] * im[j]);
            var phase = 2 * ConstantsFp32.PI * Random.Shared.NextSingle();

            filteredRe[j] = mag * MathF.Cos(phase);
            filteredIm[j] = mag * MathF.Sin(phase);
        }
    }
}
