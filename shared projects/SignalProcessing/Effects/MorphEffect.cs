...
namespace Vorcyc.Mathematics.SignalProcessing.Effects
{
    ...
    public class MorphEffect : AudioEffect
    {
        ...
        public MorphEffect(int hopSize, int fftSize = 0)
        {
            ...
            _window = WindowBuilder.OfType(WindowType.Hann, _fftSize);
        ...
        protected void ProcessFrame()
        {
            ...
            {
                var mag1 = MathF.Sqrt(_re1[j] * _re1[j] + _im1[j] * _im1[j]);
                var phase2 = MathF.Atan2(_im2[j], _re2[j]);

                _filteredRe[j] = mag1 * MathF.Cos(phase2);
                _filteredIm[j] = mag1 * MathF.Sin(phase2);
            }
