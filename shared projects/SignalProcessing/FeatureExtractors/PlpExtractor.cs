...
namespace Vorcyc.Mathematics.SignalProcessing.FeatureExtractors
{
    ...
    public class PlpExtractor : FeatureExtractor
    {
        ...
        /// </summary>
        protected readonly float[] _centerFrequencies;

        /// <summary>
        /// RASTA coefficient (if zero, then no RASTA filtering).
        /// </summary>
        protected readonly float _rasta;
        ...
        /// </summary>
        protected readonly float[] _equalLoudnessCurve;
        ...
        public PlpExtractor(PlpOptions options) : base(options)
        {
            ...
            else
            {
                ...
                {
                    var herzResolution = (float)SamplingRate / _blockSize;

                    // try to determine center frequencies automatically from filterbank weights:

                    _centerFrequencies = new float[filterbankSize];
            ...
            _equalLoudnessCurve = new float[filterbankSize];

            for (var i = 0; i < _centerFrequencies.Length; i++)
            {
                var level2 = _centerFrequencies[i] * _centerFrequencies[i];

                _equalLoudnessCurve[i] = MathF.Pow(level2 / (level2 + 1.6e5f), 2) * ((level2 + 1.44e6f) / (level2 + 9.61e6f));
            }
            ...
            var freq = ConstantsFp32.PI / (bandCount - 1);
            ...
            for (var i = 0; i < _idftTable.Length; i++)
            {
                ...
                {
                    _idftTable[i][j] = 2 * MathF.Cos(freq * i * j);
                }

                _idftTable[i][bandCount - 1] = MathF.Cos(freq * i * (bandCount - 1));
            }
            ...
            _lifterCoeffs = _lifterSize > 0 ? WindowBuilder.Liftering(FeatureCount, _lifterSize) : null;
        ...
        public override void ProcessFrame(float[] block, float[] features)
        {
            ...
            if (_rasta > 0)
            {
                ...
                {
                    var log = MathF.Log(_bandSpectrum[k] + float.Epsilon);

                    log = _rastaFilters[k].Process(log);

                    _bandSpectrum[k] = MathF.Exp(log);
                }
            }
            ...
            for (var k = 0; k < _bandSpectrum.Length; k++)
            {
                _bandSpectrum[k] = MathF.Pow(MathF.Max(_bandSpectrum[k], 1.0f) * _equalLoudnessCurve[k], 0.33f);
            }
            ...
            if (_includeEnergy)
            {
                features[0] = MathF.Log(MathF.Max(block.Sum(x => x * x), _logEnergyFloor));
            }
        }
