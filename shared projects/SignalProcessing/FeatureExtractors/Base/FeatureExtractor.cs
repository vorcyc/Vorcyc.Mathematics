...
public abstract class FeatureExtractor : IFeatureExtractor, IParallelFeatureExtractor
{
    ...
    /// </summary>
    public float HopDuration { get; protected set; }
    ...
    protected FeatureExtractor(FeatureExtractorOptions options)
    {
        ...
        if (options.FrameSize > 0)  // frame size has priority over frame duration 
        {
            ...
            FrameDuration = (float)FrameSize / SamplingRate;
        }
        else
        {
            FrameDuration = options.FrameDuration;
            FrameSize = (int)MathF.Round(SamplingRate * FrameDuration, MidpointRounding.AwayFromZero);
        }
        ...
        else
        {
            ...
            HopSize = (int)MathF.Round(SamplingRate * HopDuration, MidpointRounding.AwayFromZero);
        }

        _blockSize = FrameSize;
        _preEmphasis = options.PreEmphasis;
        _window = options.Window;

        if (_window != WindowType.Rectangular)
        {
            _windowSamples = WindowBuilder.OfType(_window, FrameSize);
        }
    }
    ...
    /// <param name="vectorCount">Number of feature vectors</param>
    /// <param name="startFrom">Starting time position (in seconds)</param>
    public virtual List<float> TimeMarkers(int vectorCount, float startFrom = 0f)
    {
        return Enumerable.Range(0, vectorCount)
                         .Select(x => startFrom + x * HopDuration)
                         .ToList();
