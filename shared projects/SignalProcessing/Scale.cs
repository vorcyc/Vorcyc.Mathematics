...
public static partial class Scale
{
    ...
    {
        return 10 * MathF.Log10(value / valueReference + float.Epsilon);
    }
    ...
    {
        return valueReference * MathF.Pow(10, level / 20);
    }
    ...
    {
        return 1127 * MathF.Log(herz / 700 + 1); // actually, should be 1127.01048, but HTK and Kaldi seem to use 1127
    }
    ...
    {
        return (MathF.Exp(mel / 1127) - 1) * 700;
    }
    ...
    {
        const float minHerz = 0.0f;
        const float sp = 200.0f / 3;
        const float minLogHerz = 1000.0f;
        const float minLogMel = (minLogHerz - minHerz) / sp;

        var logStep = MathF.Log(6.4f) / 27;

        return herz < minLogHerz ? (herz - minHerz) / sp : minLogMel + MathF.Log(herz / minLogHerz) / logStep;
    }
    ...
    public static float MelToHerzSlaney(float mel)
    {
        ...
        return mel < minLogMel ? minHerz + sp * mel : minLogHerz * MathF.Exp(logStep * (mel - minLogMel));
    }
    ...
    {
        return (26.81f * herz) / (1960 + herz) - 0.53f;
    }
    ...
    {
        return 1960 / (26.81f / (bark + 0.53f) - 1);
    }
    ...
    {
        return 9.26449f * MathF.Log(1.0f + herz) / (24.7f * 9.26449f);
    }
    ...
    {
        return (MathF.Exp(erb / 9.26449f) - 1.0f) * (24.7f * 9.26449f);
    }
    ...
    {
        var a440 = 440.0f * MathF.Pow(2.0f, tuning / binsPerOctave);

        return MathF.Log(16 * herz / a440, 2);
    }
    ...
    public static float LoudnessWeighting(float frequency, string weightingType = "A")
    {
        ...
        switch (weightingType.ToUpper())
        {
            case "B":
                {
                    var r = (level2 * frequency * 148693636f) /
                             (
                                (level2 + 424.36f) *
                                 MathF.Sqrt(level2 + 25122.25f) *
                                (level2 + 148693636)
                             );
                    return 20 * MathF.Log10(r) + 0.17f;
                }
            ...
            default:
                {
                    var r = (level2 * level2 * 148693636) /
                             (
                                 (level2 + 424.36f) *
                                  MathF.Sqrt((level2 + 11599.29f) * (level2 + 544496.41f)) *
                                 (level2 + 148693636)
                             );
                    return 20 * MathF.Log10(r) + 2.0f;
                }
        }
    }
}