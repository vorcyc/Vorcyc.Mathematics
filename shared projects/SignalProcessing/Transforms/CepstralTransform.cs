...
namespace Vorcyc.Mathematics.SignalProcessing.Transforms
{
    ...
    public class CepstralTransform : ITransform
    {
        ...
        /// </summary>
        private readonly float _logBase;
        ...
        /// </summary>
        private readonly float[] _unwrapped;
        ...
        public CepstralTransform(int cepstrumSize, int fftSize = 0, float logBase = ConstantsFp32.E)
        {
            Size = cepstrumSize;

            if (cepstrumSize > fftSize)
            {
                fftSize = cepstrumSize.NextPowerOf2();//  MathUtils.NextPowerOfTwo(cepstrumSize);
            }
        ...
        public float ComplexCepstrum(float[] input, float[] cepstrum, bool normalize = true)
        {
            ...
            var prevPhase = MathF.Atan2(_im[0], _re[0]);

            for (var n = 1; n < _unwrapped.Length; n++)
            {
                var phase = MathF.Atan2(_im[n], _re[n]);

                var delta = phase - prevPhase;

                if (delta > ConstantsFp32.PI)
                {
                    offset -= 2 * ConstantsFp32.PI;
                }
                else if (delta < -ConstantsFp32.PI)
                {
                    offset += 2 * ConstantsFp32.PI;
                }
            ...
            var delay = MathF.Round(_unwrapped[mid] / ConstantsFp32.PI);

            for (var i = 0; i < _re.Length; i++)
            {
                _unwrapped[i] -= ConstantsFp32.PI * delay * i / mid;

                var mag = MathF.Sqrt(_re[i] * _re[i] + _im[i] * _im[i]);

                _re[i] = MathF.Log(mag + float.Epsilon, _logBase);
                _im[i] = _unwrapped[i];
            }
        ...
        public void InverseComplexCepstrum(float[] input, float[] cepstrum, bool normalize = true, float delay = 0)
        {
            ...
            {
                var mag = _re[i];
                var phase = _im[i] + ConstantsFp32.PI * delay * i / mid;

                _re[i] = MathF.Pow(_logBase, mag) * MathF.Cos(phase);
                _im[i] = MathF.Pow(_logBase, mag) * MathF.Sin(phase);
            }
