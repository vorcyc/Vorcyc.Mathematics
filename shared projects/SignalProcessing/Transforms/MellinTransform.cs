...
public class MellinTransform : IComplexTransform
{
    ...
    public MellinTransform(int inputSize, int size, float beta = 0.5f)
    {
        ...
        var cur = -MathF.Log(size);
        var step = -cur / size;

        for (var i = 0; i < _expScale.Length; i++, cur += step)
        {
            _expScale[i] = MathF.Exp(cur);
        }
    }
    ...
    {
        VMath.InterpolateLinear(_linScale, input, _expScale, outRe);

        for (var i = 0; i < outRe.Length; i++)
        {
            outRe[i] *= MathF.Pow(_expScale[i], _beta);
            outIm[i] = 0;
        }
    ...
    public void DirectNorm(float[] input, float[] outRe, float[] outIm)
    {
        ...
        var norm = (1 / MathF.Sqrt(outRe.Length));
